...
def classify_intent(text: str) -> str:
    ...
    article_mentions = extract_article_mentions(t)
    if len(article_mentions) >= 2 and contains_any(t, ["so sánh", "khác gì", "phân biệt", "khác nhau", "điểm khác", "giống nhau"]):
...
class SemanticParser:
    def parse(self, question: str) -> Dict[str, object]:
        ...
        article_numbers = extract_article_mentions(normalized)
        for art in article_numbers:
            query_terms.append(f"điều {art}")
        ...
        return {
            ...
            "article_numbers": unique_keep_order(article_numbers),
